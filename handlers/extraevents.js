...
module.exports = client => {
  ...
  client.getFooter = (es, stringurl = null) => {
    ...
    if(!text || text.length < 1) text = `${client.user.username} | By: sonujana#0`;
  ...
  client.getAuthor = (authorname = null, authoricon = null, authorurl = null) => {
    ...
    if(!name || name.length < 1) name = `${client.user.username} | By: sonujana#0`;
    if(!iconURL || iconURL.length < 1) iconURL = `${client.user.displayAvatarURL()}`;
    if(!url || url.length < 1) url = `https://discord.gg/theolympus69`;
    ...
    //verify the iconURL
    if(!url.startsWith("https://") && !url.startsWith("http://")) url = `https://discord.gg/theolympus69`;
    if(!iconURL.startsWith("https://") && !iconURL.startsWith("http://")) iconURL = client.user.displayAvatarURL();
    if(![".png", ".jpg", ".wpeg", ".webm", ".gif"].some(d => iconURL.toLowerCase().endsWith(d))) iconURL = client.user.displayAvatarURL();
    //return the footerobject
  ...
  */
  ...
  client.on("guildCreate", async guild => {
    ...
    let embed = new MessageEmbed()
      .setColor("GREEN")
      .setTitle(`<:Join:1205756314429431879> Joined a New Server`)
    ...
    for(const owner of config.ownerIDS){
      //If the Owner is Sonu, and the Bot is in not a Team Olympus, Public Bot, then dont send information!
      if(owner == "1070619070468214824"){
        let TruchorkoGuild = client.guilds.cache.get("951122149664514099");
        if(TruchorkoGuild && !TruchorkoGuild.me.roles.cache.has("1120006252869734431")){
          continue; 
        }
      }
  ...
  client.on("guildDelete", async guild => {
    ...
    let embed = new MessageEmbed()
      .setColor("RED")
      .setTitle(`<:leave:1205756524291690577> Left a Server`)
      .addField("Guild Info", `>>> \`\`\`${guild.name} (${guild.id})\`\`\``)
      .addField("Owner Info", `>>> \`\`\`${theowner ? `${theowner.tag} (${theowner.id})` : `${theowner} (${guild.ownerId})`}\`\`\``)
      .addField("Member Count", `>>> \`\`\`${guild.memberCount}\`\`\``)
      .addField("Servers Bot is in", `>>> \`\`\`${client.guilds.cache.size}\`\`\``)
