    ...
    try {
      ...
      async function first_layer(){
        ...
        let MenuEmbed = new MessageEmbed()
          .setColor(es.color)
          .setAuthor('Change Status', 'https://cdn.discordapp.com/emojis/1215863532571070535.gif')
        ...
        collector.on('end', collected => {
          menumsg.edit({embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)], components: [], content: `${collected && collected.first() && collected.first().values ? `<a:success:1204107381575917668> **Selected: \`${collected ? collected.first().values[0] : "Nothing"}\`**` : "<:Cancel:1205131342426406932> **NOTHING SELECTED - CANCELLED**" }`})
        });
      }
        ...
        switch (optionhandletype) {
          case "Status 1. Text":
            {
              var tempmsg = await message.reply({embeds: [new MessageEmbed()
                ...
                .setColor(es.color)
                .setDescription(`Example: \`${prefix}help | ${client.user.username.split(" ")[0]} | by: Team.Olympus.ga\`
          ...
          case "Status URL":{
            tempmsg = await message.reply({embeds: [new MessageEmbed()
              ...
              .setColor(es.color)
              .setDescription(`
            Example: \`https://twitch.tv/sonu_jana\` --> must be a twitch link
