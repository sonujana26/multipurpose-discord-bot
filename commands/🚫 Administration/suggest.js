        ...
        try {
          ...
          if (([...message.member.roles.cache.values()] && !message.member.roles.cache.some(r => cmdroles.includes(r.id))) && !cmdroles.includes(message.author.id) && ([...message.member.roles.cache.values()] && !message.member.roles.cache.some(r => adminroles.includes(r ? r.id : r))) && !Array(message.guild.ownerId, config.ownerid).includes(message.author.id) && !message.member.permissions.has([Permissions.FLAGS.ADMINISTRATOR]))
            ...
            client.settings.ensure(message.guild.id, {
              suggest: {
                channel: "",
                approvemsg: `<a:success:1204107381575917668> Accepted Idea! Expect this soon.`,
                denymsg: `<:cx_cross:1198077504599117834> Thank you for the feedback, but we are not interested in this idea at this time.`,
                maybemsg: `💡 We are thinking about this idea!`,
                duplicatemsg: `💢 This is a duplicated Suggestion`,
                soonmsg: `👌 Expect this Feature Soon!`,
                statustext: `<a:duration:1142776994711478302> Waiting for Community Feedback, please vote!`,
                footertext: `Want to suggest / Feedback something? Simply type in this channel!`,
                approveemoji: `1206791887692898326`,
                denyemoji: `1204106928675102770`,
              }
            });
            ...
            const embed = new MessageEmbed()
                ...
                .setColor(color)
                .setFooter(client.getFooter(`Want to suggest something? Simply type it in this channel`, "https://cdn.discordapp.com/emojis/1205341157253586964.png"))
              
            if(embed.fields[2]){
              embed.fields[2].name == `<:GreenArrow:1212593348280324156> __Reason by **${message.author.tag}**:__`;
              embed.fields[2].value == `>>> ${String(reason).substr(0, 1000)}`;
            } else {
              embed.addField(`<:GreenArrow:1212593348280324156> __Reason by **${message.author.tag}**__`, `>>> ${String(reason).substr(0, 1000)}`)
            }
