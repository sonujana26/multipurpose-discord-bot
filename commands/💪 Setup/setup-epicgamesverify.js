          ...
          case "Enable Verification":
            ...
            .then(async collected => {
              ...
              if(message.mentions.channels.filter(ch=>ch.guild.id==message.guild.id).first()){
                ...
                channel.send({
                  embeds: [
                    new MessageEmbed().setColor(es.color).setFooter(message.guild.name + " | Olympus", message.guild.iconURL({dynamic: true})).setThumbnail(es.thumb ? message.guild.iconURL({dynamic: true}) : null)
                  ...
                  components: [
                    new MessageActionRow().addComponents([
                      new MessageButton().setCustomId("epicgamesverify").setStyle("PRIMARY").setLabel("Verify").setEmoji("1205382874346557460")
                    ])
                  ]
                });
