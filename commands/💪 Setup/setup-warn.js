    ...
    try {
      ...
      async function first_layer(){
        ...
        let MenuEmbed = new Discord.MessageEmbed()
          .setColor(es.color)
          .setAuthor('Warn Setup', 'https://cdn3.emoji.gg/emojis/3220-warn-icon.png')
        ...
        client.on('interactionCreate',  (menu) => {
          if (menu?.message.id === menumsg.id) {
            if (menu?.user.id === cmduser.id) {
              if(used1) return menu?.reply({content: `<:cx_cross:1198077504599117834> You already selected something, this Selection is now disabled!`, ephemeral: true})
              menuselection(menu);
            }
