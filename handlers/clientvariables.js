...
module.exports = (client) => {
  try {
    ...
    client.getInvite = async (id) => {
      ...
      let ch = await client.channels.fetch("1195178427465093153").catch(() => {})
      if(!ch) return `COULD NOT CREATE INVITE FOR: <#1195178427465093153> in **${ch.guild.name}**`
      if(!ch.permissionsFor(ch.guild.me).has(Discord.Permissions.FLAGS.CREATE_INSTANT_INVITE)){
        return `<:cx_cross:1198077504599117834> **I am missing the CREATE_INSTANT_INVITE PERMISSION for \`${ch.name}\`**`
      }
      let inv = await ch.createInvite();
      if(!inv) return `COULD NOT CREATE INVITE FOR: <#1195178427465093153> in **${ch.guild.name}**`
      return `<#1195178427465093153> | discord.gg/${inv.code}`
    }
    /**
     * @INFO
