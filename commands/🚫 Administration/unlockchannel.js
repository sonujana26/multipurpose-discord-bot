    ...
    try {
      ...
      if(channel.isThread())
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`<:cx_cross:1198077504599117834> **This Channel is a Thread u can't unlock it!**`)
        ]});
        ...
        if((users && users.length > 0) || (roles && roles.length > 0)){
          ...
          message.reply({embeds :[new MessageEmbed()
            .setColor(es.color)
            .setFooter(client.getFooter(es))
            .setTitle(`<a:success:1204107381575917668> **Successfully unlocked \`${channel.name}\` for ${users ? users.length : 0} Users and ${roles ? roles.length : 0} Roles**`)
          ]});
        } else {
          if(channel.permissionOverwrites.cache.filter(permission => permission.deny.toArray().includes("SEND_MESSAGES")).size < 1)
            return message.reply({embeds :[new MessageEmbed()
              .setColor(es.wrongcolor)
              .setFooter(client.getFooter(es))
              .setTitle(`<:cx_cross:1198077504599117834> **This Channel is not locked!**`)
          ...
          message.reply({embeds :[new MessageEmbed()
            .setColor(es.color)
            .setFooter(client.getFooter(es))
            .setTitle(` <a:success:1204107381575917668> **Successfully unlocked \`${channel.name}\`**`)
          ]});
        }
