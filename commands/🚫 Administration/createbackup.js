...
module.exports = {
    ...
    run: async (client, message, args, cmduser, text, prefix) => {
        ...
        if(!message.guild.me.permissions.has(Discord.Permissions.FLAGS.ADMINISTRATOR)){
            return message.reply("<:cx_cross:1198077504599117834> **I am missing the ADMINISTRATOR Permission!**")
        }
        ...
        if(owner.id != cmduser.id) {
            return message.reply(`<:cx_cross:1198077504599117834> **You need to be the Owner of this Server!**`)
        }
        ...
        message.channel.send({
            ...
            collector.on('collect', button => {
                if (button?.user.id === cmduser.id) {
                    collector.stop();
                    button?.reply({content: `<a:loading:1205135543071940639> **Now saving the Backup!**\nThis could take up to 2 Minutes (belongs to your data amount)`}).catch(() => {})
                    // Create the backup
