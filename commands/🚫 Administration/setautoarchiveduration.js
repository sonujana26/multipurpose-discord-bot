    ...
    try {
      ...
      if(!channel.isThread())
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`<:cx_cross:1198077504599117834> **This Channel is not a Thread**`)
        ]});
      if(!args[0])
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`<:cx_cross:1198077504599117834> **You forgot to add a Cooldown amount**`)
      ...
      message.reply({embeds :[new MessageEmbed()
        .setColor(es.color)
        .setFooter(client.getFooter(es))
        .setTitle(`<a:success:1204107381575917668> **Successfully changed the autoarchive Duration to \`${duration}\` of ${channel.name}**`)
      ]});
