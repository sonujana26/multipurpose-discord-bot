  ...
  run: async (client, interaction, cmduser, es, ls, prefix, player, message) => {
    ...
    try {
      ...
      if (boosts >= 30) maxbitrate = 384000;
        ...
        .addField(client.la[ls].cmds.info.serverinfo.field4, "<a:Channels:1205709497167781978> \`" + message.guild.channels.cache.size + "\`", true)
        .addField(client.la[ls].cmds.info.serverinfo.field5, "<a:Channel:1205214285257515089> \`" + message.guild.channels.cache.filter(channel => channel.type == "GUILD_TEXT").size + "\`", true)
        .addField(client.la[ls].cmds.info.serverinfo.field6, "<:volume_up:1204113733878546432> \`" + message.guild.channels.cache.filter(channel => channel.type == "GUILD_VOICE").size + "\`", true)
       
        .addField(client.la[ls].cmds.info.serverinfo.field7, "<:discord:1204249483475554314> \`" + message.guild.memberCount + "\`", true)
        .addField(client.la[ls].cmds.info.serverinfo.field8, "<:userRed:1205343080836759633> \`" + message.guild.members.cache.filter(member => !member.user.bot).size + "\`", true)
        .addField(client.la[ls].cmds.info.serverinfo.field9, "<:antibot:1205345894308782201> \`" + message.guild.members.cache.filter(member => member.user.bot).size + "\`", true)
        
        .addField(client.la[ls].cmds.info.serverinfo.field10, "<:online:1212050840123215943> \`" + message.guild.members.cache.filter(member => member.presence && member.presence.status != "offline").size + "\`", true)
        .addField(client.la[ls].cmds.info.serverinfo.field11, "<:offline:1212066496965255208> \`" + message.guild.members.cache.filter(member => !member.presence || member.presence.status == "offline").size + "\`", true)

        .addField(client.la[ls].cmds.info.serverinfo.field12, "<a:Boost:1212064415290105856> \`" + message.guild.premiumSubscriptionCount + "\`", true)
        .addField(client.la[ls].cmds.info.serverinfo.field13, "<:Boost_Level:1212064723152015361> \`" + boostlevel + "\`", true)
