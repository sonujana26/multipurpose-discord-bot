...
module.exports = client => {
  ...
  async function ApplySystem({ guild, channel, user, message, interaction, es, ls, preindex = false }) {
    ...
    if(!index) {
      ...
      if(!interaction.replied) return interaction?.reply({ephemeral: true, content: `<:cx_cross:1198077504599117834> Could not find the Database for your Application!`});

      else return interaction?.editReply({ephemeral: true, content: `<:cx_cross:1198077504599117834> Could not find the Database for your Application!`});
    ...
    try {
      ...
      function ask_question(qu) {
        ...
        user.send({embeds: [new Discord.MessageEmbed()
            ...
            .setAuthor(client.getAuthor(`Question ${counter + 1} / ${Questions.length}`, client.user.displayAvatarURL(), `https://discord.gg/theolympus69`))
            ...
            try {
              ...
              message.guild.channels.create(`Ticket-${usert.username}`.substr(0, 32), {
                ...
                .then((channel) => {
                  ...
                  let button_user = new MessageButton().setStyle('SUCCESS').setCustomId('ticket_user').setLabel(`Managee Users`).setEmoji(`1205343080836759633`)

                  let button_role = new MessageButton().setStyle('SUCCESS').setCustomId('ticket_role').setLabel(`Managee Roles`).setEmoji(`1213834699924049960`) 
