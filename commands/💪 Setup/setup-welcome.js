    ...
    try {
      ...
      async function first_layer(){
        ...
        let MenuEmbed = new MessageEmbed()
          .setColor(es.color)
          .setAuthor('Welcome Setup', 'https://cdn3.emoji.gg/emojis/3096_CuteGuyActuallyFlipsPaper.gif')
        ...
        switch (optionhandletype) {          
                  ...
                  async function handle_the_picks_3(optionhandletype, SetupNumber, menuoptiondata){
                    switch (optionhandletype) {
                      ...
                      case `Frame Color`:{
                        ...
                        //Once the Collections ended edit the menu message
                        collector.on('end', collected => {
                          tempmsg.edit({embeds: [tempmsg.embeds[0].setDescription(`~~${tempmsg.embeds[0].description}~~`)], components: [], content: `${collected && collected.first() && collected.first().customId ? `<:cx_tick:1198077126939779102> **Selected the \`${collected.first().customId}\` Color**` : "<:cx_cross:1198077504599117834> **NOTHING SELECTED - CANCELLED**" }`})
                        });
                        //Menu Collections
          ...
          case "Direct Welcome Messages":{
            ...
            async function second_layer(){
              let menuoptions = [
                {
                  ...
                  emoji: "1204107832232775730" //
                },
                {
                  value: "Manage the Image",
                  description: `Manage the Welcome Image for the Message`,
                  emoji: "1197765900829470721"
                },
                {
                  value: "Edit the Message",
                  description: `Edit the Welcome Message ...`,
              ...
              let MenuEmbed = new MessageEmbed()
                .setColor(es.color)
                .setAuthor('DM - Welcome Setup', 'https://cdn3.emoji.gg/emojis/3096_CuteGuyActuallyFlipsPaper.gif')
                .setDescription(eval(client.la[ls]["cmds"]["setup"]["setup-ticket"]["variable2"]))
              //send the menu msg
              let menumsg = await message.reply({embeds: [MenuEmbed], components: [new MessageActionRow().addComponents(Selection)]})
              //Create the collector
          ...
          case `Test Welcome`:{
            ...
            let invitemessage = `Invited by ${member.user.tag ? `**${member.user.tag}**` : `<@${member.user.id}>`}\n<:invites:1206172223983980554> **X Invites**\n[<a:a_member:1206172451260735509> X Joins |<:labs_left:1206172700020707369> X Leaves |<:cx_cross:1198077504599117834> X Fakes]`
            if(welcome){
              let themessage = String(welcome.secondmsg);
              if(!themessage || themessage.length == 0) themessage = "<a:welcome:1206120466721542224> {user} **Welcome to our Server!** <a:Celebrate_Left:1204295443836702811>";
