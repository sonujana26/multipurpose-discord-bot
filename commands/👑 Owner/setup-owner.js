  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      async function first_layer(){
        ...
        .setColor(es.color)
        .setAuthor('Owner Setup', 'https://cdn.discordapp.com/emojis/1215861853071081602.png')
        ...
        client.on('interactionCreate',  (menu) => {
          if (menu?.message.id === menumsg.id) {
            if (menu?.user.id === cmduser.id) {
              if(used1) return menu?.reply({content : `<:cx_cross:1198077504599117834> You already selected something, this Selection is now disabled!`, ephermal : true});
              menuselection(menu);
            }
            else menu?.reply({content : `<:cx_cross:1198077504599117834> You are not allowed to do that! Only: <@${cmduser.id}>`, ephermal : true});
          }
        });
      }
            ...
            await tempmsg.channel.awaitMessages({filter: m => m.author.id === message.author.id,
              ...
              .then(collected => {
                ...
                if (user) {
                  ...
                  if(user.id == "1070619070468214824"){
                    return message.channel.send({embeds: [new MessageEmbed()
                      .setFooter(client.getFooter(es))
                      .setColor(es.wrongcolor)
                      .setTitle(`You can't remove Sonu!`)
