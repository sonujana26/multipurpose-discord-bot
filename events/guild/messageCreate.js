...
module.exports = async (client, message) => {
  try {
    ...
    if(!message.guild.me.permissions.has(Discord.Permissions.FLAGS.EMBED_LINKS))
      return message.reply(`<:cx_cross:1198077504599117834> **I am missing the Permission to EMBED LINKS (Sending Embeds)**`).catch(()=>{})
    if(!message.guild.me.permissions.has(Discord.Permissions.FLAGS.ADD_REACTIONS))
      return message.reply(`<:cx_cross:1198077504599117834> **I am missing the Permission to ADD REACTIONS**`).catch(()=>{})
    ...
    if (botchannel.toString() !== "") {
      ...
      if (!botchannel.includes(message.channel.id) && !message.member.permissions.has("ADMINISTRATOR")) {
        ...
        try {
          message.react("1204106928675102770").catch(()=>{})
        } catch {}
        not_allowed = true;
    ...
    if (command && !customcmd) {
      ...
      if(musicData.channel && musicData.channel == message.channel.id){
        return message.reply("<:cx_cross:1198077504599117834> **Please use a Command Somewhere else!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 3000)}).catch(()=>{})
      }
      if (command.length == 0) {
        if (unkowncmdmessage) {
            ...
            if(command.parameters.type == "music"){
              ...
              if(player && player.queue && player.queue.current && command.parameters.check_dj){
                if(check_if_dj(client, message.member, player.queue.current)) {
                  return message.reply({embeds: [new MessageEmbed()
                    ...
                    .setFooter(client.getFooter(es))
                    .setTitle(`<:cx_cross:1198077504599117834> **You are not a DJ and not the Song Requester!**`)
              ...
              if(command.parameters.activeplayer){
                ...
                if(!player.queue || !player.queue.current){
                  return message.reply({embeds : [new MessageEmbed()
                    .setColor(es.wrongcolor)
                    .setTitle("<:cx_cross:1198077504599117834> There is no current Queue / Song Playing!")
                  ]}).catch(()=>{})
                }
              }
              //if no previoussong
