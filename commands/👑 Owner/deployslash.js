    ...
    if (message.author.id != "1070619070468214824")
      return message.channel.send({
        embeds: [new MessageEmbed()
          ...
          .setDescription(`Contact to the [Developer](https://discord.com/invite/Xf8U8DVpmd), and ask for it!`)
        ]
      });
    try {
      ...
      if(loadSlashsGlobal){
        let themsg = await message.reply(`<a:loading:1205135543071940639> **Attempting to set the Global Slash Commands in \`${client.guilds.cache.size} Guilds\`...**`)
        ...
        let guild = client.guilds.cache.get(guildId);
        let themsg = await message.reply(`<a:loading:1205135543071940639> **Attempting to set the GUILD Slash Commands in \`${guild.name}\`...**`)
