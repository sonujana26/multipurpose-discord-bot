    ...
    try {
      ...
      async function first_layer(){
        ...
        let MenuEmbed = new MessageEmbed()
          .setColor(es.color)
          .setAuthor('Leave Setup', 'https://cdn3.emoji.gg/emojis/7097-leave-icon.png')
                  ...
                  async function handle_the_picks_3(optionhandletype, SetupNumber, menuoptiondata){
                    switch (optionhandletype) {
                      ...
                      case `Frame Color`:{
                        ...
                        //Once the Collections ended edit the menu message
                        collector.on('end', collected => {
                          message.reply({embeds: [tempmsg.embeds[0].setDescription(`~~${tempmsg.embeds[0].description}~~`)], components: [], content: `${collected && collected.first() && collected.first().customId ? `<:cx_tick:1198077126939779102> **Selected the \`${collected.first().customId}\` Color**` : "<:cx_cross:1198077504599117834> **NOTHING SELECTED - CANCELLED**" }`})
                        });
                        //Menu Collections
