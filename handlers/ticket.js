...
module.exports = (client) => {
  ...
  for(let i = 1; i<=maxTicketsAmount; i++) {
    ...
    client.on("interactionCreate", async (interaction) => {
      ...
      let optionsData = {
        topic: `📨 #${String(filename).replace("ticket", "").length > 0 ? String(filename).replace("ticket", "") : "1"} Ticket for: ${user.tag} (${user.id}) | <:cx_tick:1198077126939779102> Created at: ${moment().format("LLLL")}`,
        type: "GUILD_TEXT",
      ...
      guild.channels.create(channelname.substr(0, 31), optionsData).then(async ch => {
        ...
        if(ticket.claim.enabled){
          var claimEmbed = new MessageEmbed()
            ...
            .setAuthor(client.getAuthor(`A Staff Member will claim the Ticket soon!`, "https://cdn.discordapp.com/emojis/1205135543071940639.gif", "https://discord.gg/theolympus69"))
        ...
        let button_close = new MessageButton().setStyle('PRIMARY').setCustomId('ticket_close').setLabel('Close').setEmoji("1212327219775414272")
        let button_delete = new MessageButton().setStyle('SECONDARY').setCustomId('ticket_delete').setLabel("Delete").setEmoji("1212327410918367253")
        ...
        let button_role = new MessageButton().setStyle('SUCCESS').setCustomId('ticket_role').setLabel("Roles").setEmoji("1204244180399034439")
        ...
        interaction?.reply({content: "<:cx_cross:1198077504599117834> **Something went wrong!**", ephemeral: true})
        console.log(e)
      })
    });
  }
