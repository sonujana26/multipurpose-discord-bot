...
module.exports = {
  ...
  description: `Mutes a Member from Guild`,
  usage: `Mute @User <Timespan> [Reason] | e.g: mute @User 10h+20min Stop spamming!`,
  type: "member",
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      if(kickmember.communicationDisabledUntilTimestamp) return message.reply(" <:cx_cross:1198077504599117834> **This User is already timeouted!**");
      let time = 0;
      if(!args[1]) return message.reply(`<:cx_cross:1198077504599117834> **No time added!**\nTry something like this:\n> \`${prefix}mute ${kickmember.id} 1h+15min Stop spamming!\``)
      let timeargs = [args[1]];
      ...
      if(!time || isNaN(time)) return message.reply(`<:cx_cross:1198077504599117834> **You added a invalid time!**\nTry something like this:\n> \`${prefix}mute ${kickmember.id} 1h+15min Stop spamming!\``)
      ...
      if (!kickmember.manageable)
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle("<:cx_cross:1198077504599117834> **I am not able to manage this User**")
        ]}).catch(()=>{});
      try{
      ...
      try {
        kickmember.timeout(time, reason).then(() => {
          ...
          message.reply({embeds: [new MessageEmbed()
            .setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
            .setFooter(client.getFooter(es))
            .setTitle(`<a:success:1204107381575917668> **${kickmember.user.tag}** got timeouted by \`${message.author.tag}\` for ${duration(time).map(t => `\`${t}\``).join(" ")}`)
            .setDescription(`Reason:\n>>> ${reason}`.substring(0, 2048))
