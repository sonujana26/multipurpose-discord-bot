...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      var button_forward = new MessageButton().setStyle('PRIMARY').setCustomId('notes_forwards').setEmoji('1204295805087784960').setLabel("Forwards")
      var button_back = new MessageButton().setStyle('PRIMARY').setCustomId('notes_backwards').setEmoji("1204295774469496863").setLabel("Backwards")
      ...
      var button_disable = new MessageButton().setStyle('SECONDARY').setCustomId('notes_disable').setLabel('Stop the Buttons').setEmoji("1205131342426406932");
      ...
      if(!notes || notes.length == 0){
          ...
          .setTitle(`<:cx_cross:1198077504599117834> No Notes created yet`)
      ...
      try{
        if(b?.user.id !== message.author.id)
          ...
          if(b?.customId == "notes_disable") {
            ...
            var buttonRow2 = new MessageActionRow().addComponents([button_create, button_edit, button_Delete, button_disable])
            allbuttons = [buttonRow1, buttonRow2]
            await b?.reply("<:cx_cross:1198077504599117834> **Disabled the Buttons**", true);
          ...
          if(b?.customId == "notes_jump") {
            ...
            await mmmmm.channel.awaitMessages({filter: m=>m.author.id == cmduser.id, max: 1, time: 180e3}).then(async collected => {
              ...
              if(Page < 0 || Page > embeds.length - 1){
                return message.reply("<:cx_cross:1198077504599117834> **Value out of Range!**").then(msg=>{
                  setTimeout(()=>{
                    try{msg.delete()}catch(e){console.log(String(e).grey)}
                  }, 2500)
                }).catch(e=>{
