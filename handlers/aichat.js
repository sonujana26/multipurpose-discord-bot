        ...
        try{
            ...
            if(message.channel.id == chatbot){
              ...
              try{
                ...
                message.channel.send({content: "<:cx_cross:1198077504599117834> AI CHAT API IS DOWN"}).catch(() => {})
              }
            }
    ...
    try{
        ...
        for(const user of [...message.mentions.users.values()]){
            if(client.afkDB.has(message.guild.id + user.id)){
                await message.reply({content: `<:RoneDo:1213789826462126120> **${user.tag}** went AFK <t:${Math.floor(client.afkDB.get(message.guild.id+user.id, "stamp") / 1000)}:R>!${client.afkDB.get(message.guild.id+user.id, "message") && client.afkDB.get(message.guild.id+user.id, "message").length > 1 ? `\n\n__His Message__\n>>> ${String(client.afkDB.get(message.guild.id+user.id, "message")).substr(0, 1800).split(`@`).join(`\`@\``)}` : "" }`}).catch(() => {})
            }
        }
    }catch(e){
...
client.on("messageCreate", async message => {
    try{
        ...
        if(message.content && !message.content.toLowerCase().startsWith("[afk]") && client.afkDB.has(message.guild.id + message.author.id)){
            ...
            await message.reply({content: `<a:Celebrate_Left:1204295443836702811> Welcome back **${message.author.username}!** <:catsip:1206789338353115136>\n> You went AFK <t:${Math.floor(client.afkDB.get(message.guild.id+message.author.id, "stamp") / 1000)}:R>`}).catch(() => {})
                    ...
                    try { 
                        if(!message.deleted) {
                            if(message.channel.permissionsFor(message.channel.guild.me).has(Permissions.FLAGS.MANAGE_MESSAGES)){
                                ...
                                message.reply("<:cx_cross:1198077504599117834> **I am missing the MANAGE_MESSAGES Permission!**").then(m => {
