...
///////////////////////////////////////
      ...
      if(!message.guild.me.permissions.has(Discord.Permissions.FLAGS.ADMINISTRATOR)){
        return message.reply("<:ml_cross:1204106928675102770> **I am missing the ADMINISTRATOR Permission!**")
      }
      ...
      if(owner.id != cmduser.id) {
          return message.reply(`<:ml_cross:1204106928675102770> **You need to be the Owner of this Server!**`)
      }
      ...
      async function first_layer(){
        let menuoptions = [
          {
            ...
            emoji: !client.settings.get(message.guild.id, "autobackup") ? "1204107832232775730" : "1204107662392827904"
          },
          {
            value: "Cancel",
        ...
        let MenuEmbed = new Discord.MessageEmbed()
          ...
          "https://cdn.discordapp.com/emojis/1211859312637644820.gif")
        ...
        collector.on('collect', menu => {
          ...
          else menu?.reply({content: `<:ml_cross:1204106928675102770> You are not allowed to do that! Only: <@${cmduser.id}>`, ephemeral: true});
        });
        //Once the Collections ended edit the menu message
        collector.on('end', collected => {
          menumsg.edit({embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)], components: [], content: `${collected && collected.first() && collected.first().values ? `<:cx_tick:1198077126939779102> **Selected: \`${collected ? collected.first().values[0] : "Nothing"}\`**` : "<:ml_cross:1204106928675102770> **NOTHING SELECTED - CANCELLED**" }`})
        });
      }
