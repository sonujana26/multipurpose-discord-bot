...
module.exports = {
  ...
  aliases: ["close", "manageticket", "ticketclose"],
  cooldown: 2,
  usage: "ticket",
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      if (([...message.member.roles.cache.values()] && !message.member.roles.cache.some(r => cmdroles.includes(r.id))) && !cmdroles.includes(message.author.id) && ([...message.member.roles.cache.values()] && !message.member.roles.cache.some(r => adminroles.includes(r ? r.id : r))) && !Array(message.guild.ownerId, config.ownerid).includes(message.author.id) && !message.member.permissions.has([Permissions.FLAGS.ADMINISTRATOR]) && !message.member.roles.cache.some(r => ticket.adminroles.includes(r ? r.id : r)))
        ...
        let button_user = new MessageButton().setStyle('SUCCESS').setCustomId('ticket_user').setLabel("Manage Users").setEmoji("1212327949089509396")
        let button_role = new MessageButton().setStyle('SUCCESS').setCustomId('ticket_role').setLabel("Manage Roles").setEmoji("1212328174780817409") 
        ...
        message.reply({embeds: [new MessageEmbed()
            ...
            .setDescription(`> <:ticket_close:1212327219775414272> **== Close** the Ticket
            
>  <:delete_ticket:1212327410918367253> **== Delete** the Ticket

> <:transcript:1212327654347505744> **== Create a Log** of the Ticket

> <:management:1212327949089509396> **==** Manage **User** Access (Add/Remove)

> <:role_manager:1212328174780817409> **==** Manage **Role** Access (Add/Remove)`).setFooter(client.getFooter(es))
