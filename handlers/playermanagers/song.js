...
async function song(client, message, args, type, slashCommand, extras) {
  ...
  if (state !== "CONNECTED") {
    ...
    player.connect();
    try{message.react("1214176314488266763").catch(() => {});}catch(e){console.log(String(e).grey)}
    player.stop();
  }
  try {
  ...
  async function playlist_() {
    ...
    var playlistembed = new MessageEmbed()
      ...
      .setThumbnail(`https://img.youtube.com/vi/${res.tracks[0].identifier}/mqdefault.jpg`)
      .addField("<a:duration:1142776994711478302> Duration: ", `\`${format(res.playlist.duration)}\``, true)
      .addField("<:length:1205774052959068240> Queue length: ", `\`${player.queue.length} Songs\``, true)
