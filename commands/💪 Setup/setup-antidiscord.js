    ...
    try {
      ...
      async function first_layer(){
        let menuoptions = [
          {
            ...
            emoji: `${client.settings.get(message.guild.id, `antidiscord.enabled`) ? "1204107832232775730" : "1204107662392827904"}`
          },
          {
            value: "Settings",
        ...
        collector.on('end', collected => {
          menumsg.edit({embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)], components: [], content: `${collected && collected.first() && collected.first().values ? `<:cx_tick:1198077126939779102> **Selected: \`${collected ? collected.first().values[0] : "Nothing"}\`**` : "❌ **NOTHING SELECTED - CANCELLED**" }`})
        });
      }
