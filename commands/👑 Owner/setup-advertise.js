  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      async function first_layer(){
        let menuoptions = [
          {
            ...
            description: `${client.ad.enabled? "Disables the Ads from Bero-Host and Sonu" : "Enables the Ads from Bero-Host and Sonu"}`,
        ...
        .setColor(es.color)
        .setAuthor('Advertising Setup', 'https://cdn.discordapp.com/emojis/1215861288538996736.png')
        ...
        client.on('interactionCreate',  (menu) => {
          if (menu?.message.id === menumsg.id) {
            if (menu?.user.id === cmduser.id) {
              if(used1) return menu?.reply({content : `<a:Warnflash:1198208227746259095> You already selected something, this Selection is now disabled!`}, {ephermal : true});
              menuselection(menu);
            }
            else menu?.reply({content : `<a:Warnflash:1198208227746259095> You are not allowed to do that! Only: <@${cmduser.id}>`}, {ephermal : true});
          }
        });
      }
