...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
        ...
        .addComponents(
          new MessageButton().setStyle("SECONDARY").setCustomId("1").setEmoji("1204594745070850158"),
          new MessageButton().setStyle("SECONDARY").setCustomId("2").setEmoji("1204594782588899361"),
          new MessageButton().setStyle("SECONDARY").setCustomId("3").setEmoji("1204594885316050945"),
          new MessageButton().setStyle("SECONDARY").setCustomId("4").setEmoji("1204594974990278790"),
        )
        ...
        collector.on('collect', async button => {
              ...
              await tempmsg.channel.awaitMessages({filter: m => m.author.id === message.author.id,
                ...
                .then(collected => {
                  ...
                  try {
                    ...
                    es = client.settings.get(message.guild.id, "embed")
                    return message.reply({embeds: [new Discord.MessageEmbed()
                      .setTitle(`<a:success:1204107381575917668> The new Embed Footer Text is:`.substr(0, 256))
                      .setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
          ...
          else button?.reply({content: `<:cx_cross:1198077504599117834> You are not allowed to do that! Only: <@${cmduser.id}>`, ephemeral: true});
        });
        //Once the Collections ended edit the menu message
        collector.on('end', collected => {
          tempmsg.edit({embeds: [tempmsg.embeds[0].setDescription(`~~${tempmsg.embeds[0].description}~~`)], components: [], content: `${collected && collected.first() && collected.first().customId ? `<:cx_tick:1198077126939779102> **Selected the \`${collected.first().customId}\`. Button**` : "<:cx_cross:1198077504599117834> **NOTHING SELECTED - CANCELLED**" }`})
        });
