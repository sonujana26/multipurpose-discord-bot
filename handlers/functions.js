...
function GetUser(message, arg){
  var errormessage = "<:cx_cross:1198077504599117834> I failed finding that User...";
...
function GetRole(message, arg){
  var errormessage = "<:cx_cross:1198077504599117834> I failed finding that Role...";
  return new Promise(async (resolve, reject) => {
    var args = arg, client = message.client;
    if(!client || !message) return reject("CLIENT IS NOT DEFINED")
    if(!args || args == null || args == undefined) args = message.content.trim().split(/ +/).slice(1);
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if(!user && args[0] && args[0].length == 18) {
      ...
      for(const g of guilds){
        var members = g.members.cache.map(this_Code_is_by_Tomato_6966 => this_Code_is_Tomato_6966);
...
async function swap_pages(client, message, description, TITLE) {
  ...
  collector.on('collect', async b => {
      if(b?.user.id !== message.author.id)
        return b?.reply({content: `<:cx_cross:1198077504599117834> **Only the one who typed ${prefix}help is allowed to react!**`, ephemeral: true})
        //page forward
        if(b?.customId == "1") {
...
async function swap_pages2(client, message, embeds) {
  ...
  let cmduser = message.author;
  if (embeds.length === 1) return message.channel.send({embeds: [embeds[0]]}).catch(e => console.log("THIS IS TO PREVENT A CRASH"))
  let button_back = new MessageButton().setStyle('SUCCESS').setCustomId('1').setEmoji("1204295774469496863").setLabel("Back")
  let button_home = new MessageButton().setStyle('DANGER').setCustomId('2').setEmoji("1204297974835056714").setLabel("Home")
  let button_forward = new MessageButton().setStyle('SUCCESS').setCustomId('3').setEmoji('1204295805087784960').setLabel("Forward")
  let button_blank = new MessageButton().setStyle('SECONDARY').setCustomId('button_blank').setLabel("\u200b").setDisabled();
  let button_stop = new MessageButton().setStyle('DANGER').setCustomId('stop').setEmoji("1204106928675102770").setLabel("Stop")
  const allbuttons = [new MessageActionRow().addComponents([button_back, button_home, button_forward, button_blank, button_stop])]
    ...
    if (guildid) {
      ...
      dbEnsure(client.settings, guildid, {
        ...
        welcome: {
          ...
          secondmsg: "<a:wavehi:1214848371844718647> {user} **Welcome to our Server!** <a:cutelove:1213438382491836427>",
          ...
          membercount: true,
          servername: true,
          msg: "{user} Welcome to this Server <a:cutelove:1213438382491836427>",
...
function ensure_economy_user(client, guildid, userid){
    dbEnsure(client.economy, `${guildid}-${userid}`, {
      ...
      items: {
        ...
        pensil: 0, bottle: 0, pen: 0, condom: 0, fish: 0, hamster: 0, dog: 0, cat: 0,      
      },
      black_market: {
        boost: {
          time: 0,
