    ...
    try {
      ...
      async function first_layer() {
        ...
        let MenuEmbed = new Discord.MessageEmbed()
          .setColor(es.color)
          .setAuthor('Menu Apply Setup', 'https://cdn3.emoji.gg/emojis/8020_menu.png')
        ...
        switch (optionhandletype) {
          case "Send the Config	Message": {
            ...
            if (!data || data.length < 1) {
              return message.reply("<:cx_cross:1198077504599117834> **You need to add at least 1 Open-Apply-Option**")
            }
          ...
          case "Add Apply Option": {
            ...
            collector.on('collect', menu => {
              if (menu?.user.id === cmduser.id) {
                ...
                if(index >= 0) {
                    return message.reply("<:cx_cross:1198077504599117834> **Options can't start the Same Apply System!** There is already an Option with that Application System!");
                }
                ...
                if(!apply_for_here.has(message.guild.id) || !apply_for_here.has(message.guild.id, "QUESTIONS") || apply_for_here.get(message.guild.id, "QUESTIONS").length < 1) 
                  return message.reply(`<:cx_cross:1198077504599117834> **The ${applySystemExecution}. Apply System is not setupped / has no Questions, create it first with: \`${prefix}setup-apply\`**`)
          ...
          let data = theDB.get(message.guild.id, "data");
          if (!data || data.length < 1) {
            return message.reply("<:cx_cross:1198077504599117834> **There are no Open-Apply-Options to remove**")
          }
          let embed = new MessageEmbed()
            .setColor(es.color)
            .setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
