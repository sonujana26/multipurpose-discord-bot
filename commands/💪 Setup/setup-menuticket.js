    ...
    try {
      ...
      async function first_layer() {
        ...
        let MenuEmbed = new Discord.MessageEmbed()
          .setColor(es.color)
          .setAuthor('Menu Ticket Setup', 'https://cdn3.emoji.gg/emojis/7037-orange-tickets.png')
        ...
        switch (optionhandletype) {
          case "Ticket Claim System":{
            ...
            */
            ...
            async function third_layer(SetupNumber) {
              ...
              let MenuEmbed = new Discord.MessageEmbed()
                .setColor(es.color)
                .setAuthor(SetupNumber + " Ticket Setup", "https://cdn3.emoji.gg/emojis/7037-orange-tickets.png")
          ...
          case "Send the Config	Message": {
            ...
            if (!data || data.length < 1) {
              return message.reply("<:cx_cross:1198077504599117834> **You need to add at least 1 Open-Ticket-Option**")
            }
          ...
          case "Edit Ticket Option": {
            let data = theDB.get(message.guild.id, "data");
            if (!data || data.length < 1) {
              return message.reply("<:cx_cross:1198077504599117834> **There are no Open-Ticket-Options to remove**")
            }
            let embed = new MessageEmbed()
              .setColor(es.color)
              .setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
            ...
            collector.on('end', collected => {
              menumsg.edit({
                embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)],
                components: [],
                content: `<:cx_tick:1198077126939779102> **Selected: \`${collected.size > 0 ? collected.first().values[0] : "NOTHING"}\`**`
              })
            });
          }
          break;
          ...
          collector.on('end', collected => {
            menumsg.edit({
              embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)],
              components: [],
              content: `<:cx_tick:1198077126939779102> **Selected: \`${collected.first().values[0]}\`**`
            })
          });
        }
        break;
          ...
          case "Manage Ticket Access": {
            ...
            if (collected && (collected.first().mentions.roles.size > 0 || collected.first().mentions.users.size > 0)) {
              ...
              message.reply(`<:claimed:1205749515626217532> Successfully added \`${toadd.length} Users/Roles\` and removed \`${toremove.length} Users/Roles\`\n> They are now always able to see, write and manage stuff in the TICKETS ment for them!`)
            } else {
