...
module.exports = async client => {
    ...
    client.on("interactionCreate", async interaction => {
        ...
        if(guilddata.verifychannel == interaction.channelId && interaction.customId == "epicgamesverify" && data.epic && data.epic.length > 5) {
            interaction.reply({
                ...
                components: [
                    ...
                    .addComponents(
                        [
                            new Discord.MessageButton().setStyle("PRIMARY").setEmoji("1212448034437210142").setLabel("Yes Change it!").setCustomId("epicgamesverify_f"),
                            new Discord.MessageButton().setStyle("SECONDARY").setEmoji("1212448060135841852").setLabel("No I want to keep it!").setCustomId("no"),
                        ]
                    )
                ]
            });
        ...
        user.send({
                        ...
                        new Discord.MessageSelectMenu().setMaxValues(1).setMinValues(1).setPlaceholder("Select the Platform").setCustomId("Platform").addOptions([
                            {
                                ...
                                emoji: "1214565406862352425"
                            },
                            {
                                label: "Playstation",
                                value: "psn",
                                description: "If you play on a Playstation",
                                emoji: "1214565655328595990"
                            },
                            {
                                label: "Xbox",
                                value: "xbl",
                                description: "If you play on a XBOX",
                                emoji: "1214565815949729842"
                            },
                            {
                                label: "Others",
                                value: "others",
            ...
            if(!Platform) {
                return user.send("<:cx_cross:1198077504599117834> Cancelled, due to no reaction in under 2 Minutes!")
            }
            user.send(`<:QuestionMark:1198445261610033222> **What is your EPIC GAMES Username?**\n> Make sure to send just the Username and send it 1:1 as it is \`Epicgames.com\``)
            ...
            if(!Username) {
                return user.send("<:cx_cross:1198077504599117834> Cancelled, due to not sending the Username in under 2 Minutes!")
            }
            let others = client.epicgamesDB.find(d => d.guild && d.guild == guildId && d.epic && d.epic == Username);
            if(others && others.length > 0) return user.send(`<:cx_cross:1198077504599117834> **Someone with the User-ID: \`${others.user}\` Linked their Account with this Epic Games Name!**`) 
            ...
            if(!tdata || tdata.code === 404) {
                return user.send("<:cx_cross:1198077504599117834> Could not find your Epic Games Account, please try again and make sure you send the right name!")
            }
            ...
            user.send({
                ...
                user.send("<a:success:1204107381575917668> **Successfully Linked your Account!**").catch(() => {});
            ...
            console.log(e)
            interaction.reply({
                content: "<:cx_cross:1198077504599117834> **I can't dm you... Please enable your DMS first!**",
                ephemeral: true,
            })
        });
        }
